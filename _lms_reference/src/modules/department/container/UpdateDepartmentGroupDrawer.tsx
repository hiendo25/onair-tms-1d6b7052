...
import { UpdateDepartmentGroupPayload, UpdateRootDepartmentPayload } from "../type";

export interface UpdateDepartmentGroupDrawerRef {
  open: (data: UpsertChildDepartmentFormProps["initialValues"]) => void;
  close: () => void;
}
export interface UpdateDepartmentGroupDrawerProps {
  open?: boolean;
}
const UpdateDepartmentGroupDrawer = React.forwardRef<UpdateDepartmentGroupDrawerRef, UpdateDepartmentGroupDrawerProps>(
  ({ open = false }, ref) => {
    const upsertFormRef = useRef<UpsertChildDepartmentFormRef>(null);
    const [openDrawer, setOpenDrawer] = useState(open);

    const [initialValues, setInitialValues] = useState<UpsertChildDepartmentFormProps["initialValues"]>(undefined);
    ...
    const handleUpdateDepartment: UpsertChildDepartmentFormProps["onSubmit"] = (formData) => {
      const updatePayload: UpdateDepartmentGroupPayload = {
        id: formData.id,
        code: formData.code,
        name: formData.name,
...
export default memo(UpdateDepartmentGroupDrawer);
