...
const QuestionItemSelection: React.FC<QuestionItemSelectionProps> = ({ methods, index, showFieldOther = true }) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const activeIndex = fields.findIndex((field) => field._fieldId === activeId);
  ...
  return (
    <Box component="div">
      <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
        <SortableContext items={fields.map((item) => item._fieldId)} strategy={verticalListSortingStrategy}>
            ...
            <OptionSortableItem key={field.id} id={field._fieldId}>
              <Box component="div" className="flex gap-2 w-full">
                ...
                <Box component="div" className="mt-3">
                  <IconButton onClick={() => remove(_indexOption)} className="font-normal">
                    <Iconify icon="solar:trash-bin-trash-bold" className="text-red-500" />
                  </IconButton>
                </Box>
              </Box>
            </OptionSortableItem>
...
const OptionSortableItem: React.FC<OptionSortableItemProps> = ({ id, children }) => {
  ...
  return (
    <div
      ...
      {...attributes}
      style={style}
      className={cn("bg-white border border-transparent rounded-xl overflow-hidden py-3", {
