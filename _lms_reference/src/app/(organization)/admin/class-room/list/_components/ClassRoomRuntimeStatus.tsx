...
const ClassRoomRuntimeStatus = ({ runtimeStatus, sx, ...boxProps }: ClassRoomRuntimeStatusProps) => {
  const currentStatus = runtimeStatus ?? null;

  const theme = useTheme();

  if (currentStatus === null || currentStatus === ClassRoomRuntimeStatusFilter.All) {
    return null;
  }
  ...
  const containerSx: SxProps<Theme> = Array.isArray(sx) ? [baseSx, ...sx] : sx ? [baseSx, sx] : baseSx;
...
const getRuntimeStatusStyle = (status: ClassRoomRuntimeStatusFilter, theme: Theme) => {
