...
export interface StepConfig<TStepId = number | string> {
  id: TStepId;
  label: string;
  description: string;
}

export interface StepNavigationProps<TStepId = number | string> {
  steps: StepConfig<TStepId>[];
  currentStep: TStepId;
  completedSteps: TStepId[];
  onStepClick: (stepId: TStepId) => void;
  isStepAccessible: (stepId: TStepId) => boolean;
  title?: string;
}

export default function StepNavigation<TStepId = number | string>({
  steps,
  currentStep,
  completedSteps,
  onStepClick,
  ...
  return (
    <Box sx={{ width: { xs: "100%", md: 360 }, flexShrink: 0, position: { md: "sticky" }, top: { md: 88 } }}>
      <Card
        sx={{ bgcolor: "grey.50", border: "1px solid", borderColor: "divider", boxShadow: "0 10px 40px rgba(0,0,0,0.05)" }}
      >
        <CardContent sx={{ pt: 3 }}>
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2 }}>
            <Box>
              ...
              <Typography variant="body2" sx={{ color: "text.disabled" }}>
                Bước {String(currentStep)} / {totalSteps}
              </Typography>
            </Box>
            <Chip label={`${Math.round(progress)}%`} color="primary" size="small" sx={{ fontWeight: 600 }} />
          </Box>
