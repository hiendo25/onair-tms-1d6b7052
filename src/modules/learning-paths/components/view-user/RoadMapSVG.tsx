...
import { getPeriodLayoutByIndex, MAP_TOP_Y, ROAD_WIDTH, useGenerateMap } from "./useGenerateMap";
...
const NODE_CENTER_X = ROAD_WIDTH / 2;
...
const RoadMapSVG = (props: RoadMapProps) => {
  ...
  const progressRingCircumference = 2 * Math.PI * NODE_PROGRESS_RING_RADIUS;
  ...
  const PeriodShape = ({
    ...
    const progressOffset = getProgressRingOffset(progressPercentage, progressRingCircumference);
  ...
  const Finished = ({
    ...
    <g onClick={onClick} style={{ cursor: hasFinished ? "pointer" : "not-allowed" }}>
      <path
      ...
      />
      <MultilineSvgText
        x={finishTextPosition?.x || 260}
        y={finishTextPosition?.y || 742}
        fill={hasFinished ? finishTextActiveColor : finishTextDisableColor}
        fontSize={finishTextFontSize}
        fontWeight={hasFinished ? finishTextActiveWeight : finishTextDisableWeight}
        textAnchor="middle"
  ...
  const PeriodItem = ({
    ...
    itemStatus?: "completed" | "active" | "locked";
    itemProgress?: number;
  }) => {
    const isEnabled = itemStatus ? itemStatus !== "locked" : currentStepIndex >= index;
    const isFinishedEnabled = currentStepIndex >= data.length - 1;
    ...
    if (isFinished) {
      if (renderFinished) {
        return (
          <g
            transform={`translate(${finishTranslateX}, ${finishTranslateY})`}
            onClick={onClick}
            style={{ cursor: isFinishedEnabled ? "pointer" : "not-allowed" }}
          >
      ...
      return (
        <g
          ...
          style={{ cursor: isFinishedEnabled ? "pointer" : "not-allowed" }}
        >
          <Finished title={title} hasFinished={currentStepIndex >= data.length - 1} onClick={onClick} />
        </g>
      );
    }
    if (renderItem) {
      return (
        <g transform={`translate(${x}, ${y})`} onClick={onClick} style={{ cursor: isEnabled ? "pointer" : "not-allowed" }}>
          {renderItem(index)}
        </g>
      );
    }
    return (
      ...
      >
        {(() => {
          ...
          const titleX = titlePosition?.x || NODE_CENTER_X;
          const titleY = titlePosition?.y || DEFAULT_TITLE_Y;
          const contentX = contentPosition?.x || NODE_CENTER_X;
          const contentY = contentPosition?.y || DEFAULT_CONTENT_Y;
          const progressValue = itemProgress ?? 0;
          const progressText = `${progressValue}%`;
          const titlePillWidth = getPillWidth(title, titleTextFontSize, PERIOD_TITLE_PILL_PADDING);
          ...
          return (
            <>
              ...
              />
              <g
                className="lp-roadmap-tooltip"
                style={{ pointerEvents: "none", zIndex: TOOLTIP_Z_INDEX }}
              >
                <rect
                  x={tooltipX}
                  y={tooltipY}
              ...
              <MultilineSvgText
                x={titleX}
                y={titleY}
                fill={isLocked ? titleDisableColor : titleActiveColor}
                fontSize={titleTextFontSize}
                fontWeight={isLocked ? titleDisableWeight : titleActiveWeight}
                textAnchor="middle"
                alignmentBaseline="middle"
                style={titleStyle}
              ...
              <MultilineSvgText
                x={contentX}
                y={contentY}
                fill={isLocked ? contentDisableColor : contentActiveColor}
                fontSize={contentTextFontSize}
                fontWeight={isLocked ? contentDisableWeight : contentActiveWeight}
                textAnchor="middle"
                alignmentBaseline="middle"
                style={contentStyle}
