...
const AssessmentSelector: React.FC<AssessmentSelectorProps> = ({
  ...
  const { orgId: organizationId } = useUserOrganization((state) => state.currentOrganization);
  const [searchText, setSearchText] = useState("");
  const debourceText = useDebounce(searchText, 600);
  const { data: assignments, isLoading } = useGetAssignmentsQuery({ search: debourceText, organizationId });
  ...
  return (
    <div className={cn("assessment-selector", className)}>
      ...
      >
        <Typography sx={{ fontWeight: 600, fontSize: "0.875rem", color: "black" }}>
          {selectedAssignment ? selectedAssignment.name : placeholder ? placeholder : "Chọn bài kiểm tra"}
        </Typography>
      </Button>
