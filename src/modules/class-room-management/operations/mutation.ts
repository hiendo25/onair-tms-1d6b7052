  ...
  return useTMutation({
    mutationFn: async (input: ExportStudentsMutationInput) => {
      ...
      if (!response.ok) {
        ...
        (error as Error & { severity?: "info" | "error" }).severity = response.status === 404 ? "info" : "error";
        throw error;
      }
...
export const useUpdateClassRoomMutation = () => {
  return useTMutation({
    mutationFn: (input: UpdateClassRoomDto) =>
      client.post<NonNullable<UpdateClassRoomResponse["data"]>>(`/class-rooms/${input.classRoomId}`, input),
  });
};
