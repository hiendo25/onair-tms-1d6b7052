...
const ClassRoomContainer = () => {
  ...
  return (
    <Box className="bg-white rounded-2xl p-4">
      ...
      <Stack mt={2} direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ xs: "stretch", md: "center" }}>
        <Box flex={1} sx={{ minWidth: 0 }}>
          <ClassRoomStatusTabs value={runtimeStatus} onChange={handleStatusChange} counts={runtimeStatusCounts ?? []} />
        </Box>
      </Stack>
      ...
      {showEmptyState ? (
        ...
        >
          ...
          <Typography variant="body2">Hãy thử điều chỉnh bộ lọc hoặc tìm kiếm với từ khóa khác.</Typography>
        </Box>
      ) : null}
      ...
      {!isLoading && classRooms.length > 0 ? (
        <Grid container columns={12} spacing={2} mt={4}>
          {classRooms.map((item) => {
            ...
            const { label: actionLabel, disabled: actionDisabled } = getStatusAndLabelBtnJoin(
              runtimeStatusKey,
              sessionType!,
            );
