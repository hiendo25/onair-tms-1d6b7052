...
type StudentItem = GetStudentsResponse["data"][number];
...
function renderStatus(status: "active" | "inactive") {
  ...
  return <Chip label={status === "active" ? "Đang kích hoạt" : "Chưa kích hoạt"} color={colors[status]} size="small" />;
}
...
export const columns: GridColDef<StudentItem>[] = [
  {
    field: "id",
    headerName: "ID",
    width: 60,
  ...
  },
  {
    ...
    flex: 1.5,
    minWidth: 120,
    renderCell: ({ row }) => row.employee_code,
  },
  {
    field: "student",
    ...
    flex: 1.5,
    minWidth: 240,
    renderCell: ({ row }) => row.profiles.full_name,
  },
  {
    field: "department",
