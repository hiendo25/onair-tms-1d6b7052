...
const ClassRoomCountDownSection = ({ sessionId }: ClassRoomCountDownSection) => {
  ...
  const canAccess =
    isSameOrganization &&
    (isAdmin || (isTeacher && isAssignedToSession) || (!isAdmin && !isTeacher && isAssignedToClassRoom));

  const shouldRedirectToForbidden = Boolean(!isPending && !isError && classSession && classRoom && !canAccess);
  ...
  const teachers = useMemo(() => extractTeachers(classSession), [classSession]);
  ...
  if (isError || !classSession || !classRoom) {
    return (
      <div className="mx-auto max-w-3xl rounded-2xl bg-white p-8 text-center shadow-sm mt-4">
        <h1 className="text-xl font-semibold text-gray-900">Không tìm thấy phiên học</h1>
                    ...
                    {teachers.map((teacher) => {
                      return (
                        <Tooltip key={teacher.id} title={teacher.name}>
                          <Avatar alt={teacher.name} src="/static/images/avatar/3.jpg" sx={{ width: 40, height: 40 }} />
                        </Tooltip>
                      );
                    })}
                  </AvatarGroup>
