...
export function useBranchMutations() {
  ...
  const create = useMutation({
    mutationFn: async (payload: Partial<DBBranch>) => {
      const { error } = await supabase.from("branches").insert({ ...payload, org_id: orgId } as any);
      if (error) throw error;
    },
  ...
  const bulkInsert = useMutation({
    mutationFn: async (rows: Partial<DBBranch>[]) => {
      const { error } = await supabase.from("branches").insert(rows.map((r) => ({ ...r, org_id: orgId })) as any);
      if (error) throw error;
    },
  ...
  return {
    create: useMutation({
      mutationFn: async (p: Partial<DBDepartment>) => {
        const { error } = await supabase.from("departments").insert({ ...p, org_id: orgId } as any);
        if (error) throw error;
      },
    ...
    bulkInsert: useMutation({
      mutationFn: async (rows: Partial<DBDepartment>[]) => {
        const { error } = await supabase.from("departments").insert(rows.map((r) => ({ ...r, org_id: orgId })) as any);
        if (error) throw error;
      },
      onSuccess: () => invalidate(),
    }),
  ...
  return {
    create: useMutation({
      mutationFn: async (p: Partial<DBRole>) => {
        const { error } = await supabase.from("org_roles").insert({ ...p, org_id: orgId } as any);
        if (error) throw error;
      },
